...
type StoreSettings = typeof fallbackSettings & { whatsappNumber: string };
...
function BrandMark({ compact = false, logoKey }: { compact?: boolean; logoKey?: string | null }) {
  const logo = logoKey ? assetUrl(logoKey) : "/logo-frango.png";
  return <div className={`brand-mark ${compact ? "brand-mark--compact" : ""}`}><img className="brand-mark__image" src={logo} alt="Crazy Chicken" />{!compact && <div className="brand-mark__copy"><strong>Crazy</strong><span>Chicken</span></div>}</div>;
}

function ProductCard({ product, onAdd }: { product: Product; onAdd: (product: Product) => void }) {
  return <article className="product-card"><div className="product-card__image-wrap"><img src={product.image} alt={product.name} className="product-card__image" />{product.badge && <span className="product-card__badge">{product.badge}</span>}<button type="button" className="product-card__quick-add" onClick={() => onAdd(product)} aria-label={`Adicionar ${product.name}`}><Plus size={18} strokeWidth={3} /></button></div><div className="product-card__body"><div className="product-card__meta"><span>{product.category}</span><span className="product-card__dot" /><span>feito na hora</span></div><h3>{product.name}</h3><p>{product.description}</p><div className="product-card__footer"><strong>{money(product.priceCents)}</strong><button type="button" className="product-card__add" onClick={() => onAdd(product)}>Adicionar <ArrowRight size={15} /></button></div></div></article>;
}
...
function CartDrawer({ cart, settings, zones, onClose, onChangeQuantity, onRemove, onClear }: { cart: CartItem[]; settings: StoreSettings; zones: DeliveryZone[]; onClose: () => void; onChangeQuantity: (lineId: string, delta: number) => void; onRemove: (lineId: string) => void; onClear: () => void }) {
...
export function StorefrontExperience() {
  ...
  useEffect(() => { const saved = window.localStorage.getItem("crazy-chicken-cart"); if (saved) { try { setCart(normalizeCart(JSON.parse(saved))); } catch { window.localStorage.removeItem("crazy-chicken-cart"); } } fetch("/api/storefront").then((response) => response.ok ? response.json() as Promise<{ products?: CatalogProduct[]; settings?: Partial<StoreSettings>; deliveryZones?: DeliveryZone[] }> : null).then((data) => { if (!data) return; if (Array.isArray(data.products)) setProducts(data.products.map(mapProduct)); if (Array.isArray(data.deliveryZones)) setZones(data.deliveryZones); if (data.settings) setSettings((current) => ({ ...current, ...data.settings, whatsappNumber: data.settings?.whatsappNumber ?? "", appearance: { ...current.appearance, ...(data.settings?.appearance ?? {}) } })); }).catch(() => undefined); }, []);
  ...
  useEffect(() => { const root = document.documentElement; if (settings.logoKey) root.dataset.logoKey = settings.logoKey; root.style.setProperty("--yellow", settings.appearance.accent); root.style.setProperty("--red", settings.appearance.primary); root.style.setProperty("--cream", settings.appearance.background); }, [settings]);
  ...
  const startAdd = (product: Product) => product.options.length ? setCustomizing(product) : addToCart(product);
