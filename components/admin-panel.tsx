...
import { whatsappTemplateOptions, type WhatsAppTemplate } from "../lib/whatsapp-order";
...
export function AdminPanel({ products, currentUser, initialSection = "visao" }: { products: Product[]; currentUser: CurrentAdmin; initialSection?: AdminSection }) {
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      ...
      {section === "configuracoes" && <SettingsSectionWithTemplate settings={settings} zones={zones} dirty={settingsDirty.operations} saving={savingSettings} onPatch={(patch) => patchSettings(patch, "operations")} onSave={() => saveSettings("operations")} onZones={setZones} notify={notify} upload={upload} />}
...
function SettingsSectionWithTemplate({ settings, zones, dirty, saving, onPatch, onSave, onZones, notify, upload }: { settings: AdminSettings; zones: DeliveryZone[]; dirty: boolean; saving: boolean; onPatch: (patch: Partial<AdminSettings>) => void; onSave: () => void; onZones: (zones: DeliveryZone[]) => void; notify: (message: string) => void; upload: (file: File) => Promise<string> }) {
  return <><SettingsSection settings={settings} zones={zones} dirty={dirty} saving={saving} onPatch={onPatch} onSave={onSave} onZones={onZones} notify={notify} upload={upload} /><section className="admin-card whatsapp-template-card"><div className="admin-card__heading"><div><span className="eyebrow">Mensagem do pedido</span><h2>Modelo para o WhatsApp</h2><p className="admin-help">Escolha como o pedido será organizado quando o cliente abrir a conversa.</p></div></div><div className="whatsapp-template-grid">{whatsappTemplateOptions.map((template) => <button type="button" key={template.id} className={`whatsapp-template-option ${settings.whatsappTemplate === template.id ? "active" : ""}`} aria-pressed={settings.whatsappTemplate === template.id} onClick={() => onPatch({ whatsappTemplate: template.id })}><strong>{template.label}</strong><span>{template.description}</span><small>{template.preview}</small></button>)}</div><p className="whatsapp-template-note">O modelo selecionado será usado nos próximos pedidos, inclusive quando o cliente reabrir um pedido já registrado.</p><button type="button" className="primary-button appearance-save" disabled={!dirty || saving} onClick={onSave}><Check size={17} /> {saving ? "Salvando..." : dirty ? "Salvar configurações" : "Configurações salvas"}</button></section></>;
}
